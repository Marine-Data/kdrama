...
async function avecBoutonDesactive(boutonId, texteEnCours, action) {
...
function enrichirDramasAvecPersonnes(dramasList) {
...
async function chargerDramaPersonnes() {
  try {
...
function findPersonneId(nom, role) {
  if (!nom) return null;
...
function entriesPourFichesPersonnes() {
...
function renderRechercheProfilResultats() {
...
function renderRechercheProfilModal() {
...
function hasLikedVisionnage(visionnageId) {
  const uid = currentUserId();
...
function getVisionnageLikeCount(visionnageId) {
...
async function toggleLikeAVoir(mesAVoirId, ownerId, titre) {
  if (!isLoggedIn()) { showToast("Connecte-toi pour aimer."); return; }
  const uid = currentUserId();
  const d = state.profilPublicData;
  if (!d) return;
...
function ouvrirCommentairesEntry(kind, targetId, titre, ownerId) {
...
function commentTableEtColonne(kind) {
...
async function chargerCommentairesEntry(kind, targetId) {
...
function fermerCommentairesEntry() {
  setState({ entryCommentModal: null });
}
async function publierCommentaireEntry() {
  const m = state.entryCommentModal;
  if (!m) return;
...
async function ouvrirListeAbonnes(profilId, mode) {
...
async function tenterGagnerCadeau(source) {
...
async function ouvrirEnviesAbonnes() {
...
async function piocherEnvieAbonne(aVoirId) {
  if (!isLoggedIn()) return;
  try {
...
async function trouverOuCreerAVoir(d) {
...
async function ouvrirSuggererA(d) {
...
function fermerSuggererA() {
  setState({ suggererADraft: null });
}
async function envoyerSuggestion(toUserId) {
  const draft = state.suggererADraft;
  if (!draft) return;
  try {
...
async function chargerNotificationsSuggestions() {
...
function getSuggestionsNonLues() {
...
async function chargerNotificationsAbonnements() {
...
async function chargerNotificationsInteractions() {
...
async function marquerAbonnementsVus() {
...
async function marquerInteractionsVues() {
...
async function ouvrirNotifSuggestions() {
  setState({ notifSuggestionsOpen: true });
}
function fermerNotifSuggestions() {
...
async function marquerSuggestionsVues() {
...
async function ajouterDepuisSuggestion(suggestion) {
...
function majDomResultatsRechercheProfil() {
...
async function rechercherProfilParPseudo(query) {
  const q = query.trim();
...
async function toggleLikeAvis(avisId, ownerId, titre) {
...
// ---- Likes pour les cadeaux ----
async function toggleLikeCadeau(cadeauId, ownerId, titre) {
...
function inscriptionActiviteCardHtml(p) {
...
function cadeauActiviteCardHtml(c) {
...
function renderAbonnementsView() {
  if (!isLoggedIn()) {
    return `<div class="trends-wrap"><div class="empty-state">
...
function construirePoolPourType(sourceType, dejaPlaces) {
...
async function toggleLikeTierlist(tierlistId, ownerId, titre) {
...
async function ouvrirHistoriqueQuiz() {
...
function fermerHistoriqueQuiz() {
  setState({ historiqueQuizModal: null });
}
function toggleDetailQuizPasse(date) {
...
function renderDetailQuizPasse(reponses) {
...
function renderHistoriqueQuizModal() {
...
function castingSelectionDepuisDrama(d) {
...
function castingSelectionDepuisTexteTmdb(details) {
...
function ajouterPersonneAuCasting(role, personne) {
...
function retirerPersonneDuCasting(role, index) {
  const liste = state.draft.castingSelection[role] || [];
  setState({
...
function attachCastingResultListeners(wrapper) {
...
function renderCastingSelector(role, label) {
...
async function synchroniserCastingDrama(dramaId, castingSelection) {
...
// texte dupliqué.
async function enregistrerModificationPersonne() {
...
function renderYearDramaList(year, entries) {
...
// les bons titres.
function renderTrendsSections(trends, entries, profilId) {
