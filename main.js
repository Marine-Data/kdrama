...
export function render() {
  ...
  `;
    ...
    // Sécurité : si une fonction render*() lève une erreur pendant la
    // construction de l'écran, on n'affiche PLUS un écran figé et muet.
    // On remplace le contenu par un message lisible (visible sur
    // téléphone, sans console) plutôt que de rester bloqué.
    console.error("Erreur pendant le rendu de l'écran", e);
    const msg = String((e && e.message) ? e.message : e).replace(/[<>&]/g, " ");
    root.innerHTML = `<div class="loading-screen" style="padding:24px; text-align:center;">
      <div class="app-seal motif-flower">${FLOWER_MOTIF_SVG}</div>
      <p style="font-size:13px; line-height:1.6; word-break:break-word;">
        Une erreur est survenue pendant l'affichage du carnet.<br><br>
        <span style="font-family:monospace; font-size:11px; color:#a33;">${msg}</span><br><br>
        Recharge la page. Si le problème persiste, fais une capture de cet écran.
      </p>
    </div>`;
    return;
  }
  ...
  if (headerEl) headerEl.classList.toggle("scrolled", computeHeaderScrolled());
    ...
    // Un souci d'attachement d'un écouteur ne doit PAS effacer un écran
    // déjà affiché correctement : on se contente de le signaler.
    console.error("Erreur pendant l'attachement des interactions", e);
  }
}
